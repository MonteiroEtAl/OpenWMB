...
saveWorkbook(OUT, "BDs_imput2.xlsx",overwrite = TRUE)
...
##Genrerates a DB that includes pooled estimates of the 20 DBs included in the file Imputed_Databases_Validation_PT_version based on Rubin’s (1987) rules.
para_merge_Trans <- NULL
para_merge_BD1 <- NULL
para_merge_BD2 <- NULL
para_merge_Oringinal_DB_1 <- DB_1[,-c(1,7)]
para_merge_Oringinal_DB_2 <- DB_2[,-c(1,3)]
for (i in 1:20) para_merge_Trans[[i]] <- read_excel('BDs_imput2.xlsx', sheet = excSheet[[i]])
for (i in 1:20) para_merge_BD1[[i]] <- para_merge_Trans[[i]][,c(3:7,9)]
for (i in 1:20) para_merge_BD2[[i]] <- para_merge_Trans[[i]][,c(10,12)]
